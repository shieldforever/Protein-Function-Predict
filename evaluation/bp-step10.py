...
def solve():
    ...
    for i in range(num):
        ...
        file = open(os.path.join('protein-predict-bp-list-all', todo_protein_list[i] + '-predict-bp-list-all.txt'), 'r')
        all_ans_list = []
        for line in file.readlines():
            all_ans_list.append(line[:-1])
        ans_list = all_ans_list[:length]
        test(protein=todo_protein_list[i], ans_list=ans_list, go='real_bp_go_list')
    
    file = open('bp-' + str(start_line_id) + '-' + str(end_line_id) + '-' + str(length) + '.txt', 'w')
    for x in score_list:
        file.write(x[0] + ' ' + x[1] + '\n')
    file.close()
