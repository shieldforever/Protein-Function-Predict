...
def part3():
    ...
    pf = open('parameter.txt', 'r')
    id = 0
    for l in pf.readlines():
        id = id + 1
        if id >= start_line_id and id <= end_line_id:
            ll = "".join(l)
            line = ll.split(' ')
            para_l = int(line[0])
            para_w_ = int(line[1])
            para_k = int(line[2])
            num_of_neighbor = int(line[3][:-1])
            para_w = para_w_ / 100
            print('calc line ' + str(id) + ' : ' + str(para_l) + '-' + str('%.2f' % para_w) + '-' + str(para_k) + '-' + str(num_of_neighbor))
            ...
            print('finish calc line ' + str(id) + ' : ' + str(para_l) + '-' + str('%.2f' % para_w) + '-' + str(para_k) + '-' + str(num_of_neighbor))
    ...
    #         data, pr = get_data(ultrapse_file_path=ultrapse_file_path)
    #         print(ultrapse_file_name + ' finished')
    #         for para_k in range(10, 90, 10):
    #             if para_k > 10:
    #                 break
    #             for num_of_neighbor in range(30, 160, 10):
    #                 if num_of_neighbor > 30:
    #                     break
    #                 Calc_Spectral_Clustering(data=data, pr=pr, para_l=para_l, para_w=para_w, para_k=para_k, num_of_neighbor=num_of_neighbor)
